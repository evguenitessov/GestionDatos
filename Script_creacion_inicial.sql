...
CREATE PROCEDURE [UN_CORTADO].[CANCELAR_RANGO_FECHA] (@FECHA_SISTEMA DATE,@ESPECIALIDAD_MEDICO int,@FECHA_DESDE DATE,@FECHA_HASTA DATE,
													  @DETALLE_CANCELACION VARCHAR(30), @TIPO_CANCELACION VARCHAR(30), @NOMBRE_USUARIO VARCHAR(30)) 
AS
BEGIN
	DECLARE @FECHA_AUX DATE
	DECLARE @FECHA_AUX2 DATE
	DECLARE @TURNO_PARA_CANCELACIONES INT
	...
	WHILE @FECHA_AUX <= @FECHA_HASTA
		BEGIN
			...
			BEGIN
				UPDATE [UN_CORTADO].[TURNOS] SET Disponible=2, Id_Afiliado = NULL, Hora_Llegada_Afiliado = NULL  --HAY QUE CAMBIAR EL TIPO DE DATO DE DISPONIBLE
				WHERE Id IN ( 
				...
				DECLARE C_CANCELACIONES_RANGO CURSOR FOR
					SELECT T.Id
					FROM [GD2C2016].[UN_CORTADO].[TURNOS] T
					INNER JOIN [GD2C2016].[UN_CORTADO].[AGENDA] A
					ON T.Id_Agenda= A.Id
				...
				OPEN C_CANCELACIONES_RANGO
				FETCH NEXT FROM C_CANCELACIONES_RANGO INTO @TURNO_PARA_CANCELACIONES

				WHILE (@@FETCH_STATUS = 0)
				BEGIN
					INSERT INTO [UN_CORTADO].[CANCELACIONES] ([Nombre_Usuario],[Id_Turno],[Detalle],[Tipo],[Fecha_Cancelacion])
					VALUES(@NOMBRE_USUARIO, @TURNO_PARA_CANCELACIONES, @DETALLE_CANCELACION, @TIPO_CANCELACION, @FECHA_SISTEMA)

					FETCH NEXT FROM C_CANCELACIONES_RANGO INTO @TURNO_PARA_CANCELACIONES
				END	
				CLOSE C_CANCELACIONES_RANGO
				DEALLOCATE C_CANCELACIONES_RANGO	
			END --IF
			SET @FECHA_AUX = DATEADD(DAY,1,@FECHA_AUX)
		END -- WHILE
END --PROCEDURE
...
CREATE PROCEDURE [UN_CORTADO].[CANCELAR_UN_DIA] (@FECHA_SISTEMA DATE,@ESPECIALIDAD_MEDICO int,@FECHA_CAN DATE, @DETALLE_CANCELACION VARCHAR(30),
												 @TIPO_CANCELACION VARCHAR(30), @NOMBRE_USUARIO VARCHAR(30)) 
AS
BEGIN
	DECLARE @FECHA_AUX2 DATE
	SET @FECHA_AUX2= DATEADD(DAY,1,@FECHA_SISTEMA)
	...
	BEGIN
		...
		DECLARE C_CANCELACIONES CURSOR FOR
		SELECT T.Id
			FROM [GD2C2016].[UN_CORTADO].[TURNOS] T
			INNER JOIN [GD2C2016].[UN_CORTADO].[AGENDA] A
			ON T.Id_Agenda= A.Id
		...
		OPEN C_CANCELACIONES
		FETCH NEXT FROM C_CANCELACIONES INTO @TURNO_PARA_CANCELACIONES
		...
		BEGIN
			...
			FETCH NEXT FROM C_CANCELACIONES INTO @TURNO_PARA_CANCELACIONES
		END	
		CLOSE C_CANCELACIONES
		DEALLOCATE C_CANCELACIONES
	END --IF
END --PROCEDURE
GO
